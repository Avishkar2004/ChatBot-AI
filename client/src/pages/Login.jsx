      ...
      <div className="max-w-md w-full mx-auto glass rounded-xl p-6">
        ...
        <form onSubmit={onSubmit} className="space-y-4" noValidate>
          ...
          <div>
            ...
            <input id="password" name="password" autoComplete="current-password" aria-required="true" type="password" className="input" value={password} onChange={(e) => setPassword(e.target.value)} required />
          </div>
          <button disabled={loading} className="btn-primary w-full">
            {loading ? 'Logging in...' : 'Login'}
          </button>
        </form>
        <p className="text-sm text-gray-400 mt-4">No account? <Link to="/signup" className="text-emerald-400 hover:text-emerald-300">Sign up</Link></p>
      </div>
    </div>
  );
};
